...
public class PrintABC2 {
    /*
    方法二: 锁 + 状态变量
         状态变量是一个volatile的整型变量，
         0代表打印a，1代表打印b，2代表打印c，三个线程都循环检验标志位，
         通过阻塞前和阻塞后两次判断可以确保当前打印的正确顺序，
         随后线程打印字符，然后设置下一个状态字符，唤醒其它线程，然后重新进入循环
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //打印A的线程
        Thread threadA = new Thread(new Printer(lock, 0, 1,
                'A'));
        //打印B的线程
        Thread threadB = new Thread(new Printer(lock, 1, 2,
                'B'));
        //打印C的线程
        Thread threadC = new Thread(new Printer(lock, 2, 0,
                'C'));
        //一次启动A B C线程
        threadA.start();
        Thread.sleep(1000);
